...
Директория вашей команды одна из: `//home/hackathon/team12`,
`//home/hackathon/team19`, `//home/hackathon/team25`.
...
```
echo "158.160.211.120 rp-0.rpc-proxies.hackathon.svc.cluster.local" | sudo tee -a /etc/hosts
```
...
Add-Content -Path $env:WINDIR\System32\drivers\etc\hosts `
  -Value "158.160.211.120 rp-0.rpc-proxies.hackathon.svc.cluster.local"
```

Проверка (везде одинаково):

```
python3 -c "import socket; print(socket.getaddrinfo('rp-0.rpc-proxies.hackathon.svc.cluster.local', 9013)[0][4][0])"
```

Должно напечатать `158.160.211.120`.
...
```
& $HOME\spyt-summer-school\Scripts\Activate.ps1
$env:YT_PROXY = "https://http-proxy-hackathon.demo.ytsaurus.tech/"
...
```
spark-submit \
  --master ytsaurus://https://http-proxy-hackathon.demo.ytsaurus.tech \
  --deploy-mode cluster \
  --num-executors 1 \
  --conf spark.pyspark.python=/usr/bin/python3.11 \
  --py-files yt:///home/hackathon/lib/spyt_deps.zip \
  yt:///home/example/hello.py //home/hackathon/<ваша команда>/result
```
...
```
yt read-table //home/hackathon/<ваша команда>/result --format json
```
