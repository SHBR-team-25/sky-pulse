...
│   │
│   ├───flight                                   # моковые данные рейсов для карты и карточки деталей
│   │   │   index.ts                             # публичный API моковых данных рейсов
│   │   │
│   │   └───model
...
│           api.ts                               # типы API, сгенерированные из OpenAPI-спеки (правится только генератором)
│
└───widgets                                      # слой самостоятельных блоков интерфейса
    ...
    └───flight-map                               # карта аэропортов и рейсов с маркерами, кластерами и деталями
        ...
        │
        ├───model
        │       types.ts                         # типы аэропорта, рейса и деталей для карты из OpenAPI-схемы
        │
        └───ui
